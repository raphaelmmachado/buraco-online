    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-6 font-sans">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-slate-800 rounded-2xl shadow-xl border border-slate-700 flex flex-col overflow-hidden min-h-80">
            {/* TABS */}
              ...
              {activeTab === "ONLINE" ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-4 animate-fade-in">
                  <p className="text-center text-slate-400 text-sm mb-4">
                    Jogue localmente contra a inteligência artificial. Não
                    requer conexão.
                  </p>
                  {onPlayLocal && (
                    <button
                      onClick={onPlayLocal}
                      className="w-full bg-green-600 hover:bg-green-500 text-white py-6 rounded-xl font-bold shadow-lg transition-all active:scale-95 flex items-center justify-center gap-3 text-lg"
                    >
        ...
        <div className="bg-slate-800 p-6 rounded-2xl shadow-xl border border-slate-700 flex flex-col min-h-100">
          ...
          <div className="space-y-3 overflow-y-auto max-h-100 pr-2 scrollbar-thin scrollbar-thumb-slate-600">
            {rooms.length === 0 ? (
              ...
              rooms.map((room) => {
                ...
                return (
                  <div
                    key={room.roomId}
                    className={`group bg-slate-900/50 p-4 rounded-xl border transition-all flex justify-between items-center ${
                      isFull
                        ? "border-slate-800 opacity-60"
                        : "border-slate-700 hover:border-yellow-500/50 hover:bg-slate-900"
                    }`}
                  >
                    <div>
                      <div className="flex items-center gap-2">

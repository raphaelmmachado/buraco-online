        ...
        <div className="flex flex-col gap-6 animate-fade-in">
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="space-y-2 pl-2">
                    <div className="flex items-center gap-3 text-sm">
                        <div className={`p-1 rounded ${rules.canPickUpDiscardWithJoker ? 'bg-green-500/20 text-green-400' : 'bg-white/5 text-white/20'}`}>
                            <Check size={14} strokeWidth={rules.canPickUpDiscardWithJoker ? 4 : 1} />
                        </div>
                        <span className={rules.canPickUpDiscardWithJoker ? 'text-slate-200 font-bold' : 'text-slate-500 line-through decoration-slate-700'}>
                            Comprar lixo com curinga
                        </span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                        <div className={`p-1 rounded ${rules.teamCanTakeBothDeadPiles ? 'bg-green-500/20 text-green-400' : 'bg-white/5 text-white/20'}`}>
                            <Check size={14} strokeWidth={rules.teamCanTakeBothDeadPiles ? 4 : 1} />
                        </div>
                        <span className={rules.teamCanTakeBothDeadPiles ? 'text-slate-200 font-bold' : 'text-slate-500 line-through decoration-slate-700'}>
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 gap-2 bg-black/20 rounded-2xl border border-white/5 overflow-hidden">
                    ...
                    <PointRow label="Canastra de 500 (13 cartas)" value={rules.pointsKingCanastra} />
                    <PointRow label="Canastra Real (14 cartas)" value={rules.pointsAceCanastra} />
                    <PointRow label="Morto não pego" value={rules.penaltyDeadPileNotTaken} isPenalty />
                </div>
            </div>
        </div>
    );
...
const PointRow = ({ label, value, isPenalty }: { label: string, value: number, isPenalty?: boolean }) => (
    <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
        ...
        <div className="flex items-center gap-1">
            <span className={`font-mono text-sm font-black ${isPenalty ? 'text-red-400' : 'text-yellow-500'}`}>
                {isPenalty ? '-' : ''}{value}
            </span>
...
export const GameRulesModal = ({
  ...
  const [prevExternalRules, setPrevExternalRules] = useState<GameRules>(externalRules);
      ...
      <div className="bg-[#141414] w-full max-w-md rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden animate-scale-in">
        ...
        <div className="p-6 md:p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-yellow-500/5 to-transparent">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${isHost ? 'bg-yellow-500/20 text-yellow-500' : 'bg-blue-500/20 text-blue-400'}`}>
              <Settings2 size={24} />
            </div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter text-white">Regras da Mesa</h2>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                {isHost ? "Ajustes salvos automaticamente" : "Consultando configurações"}
              </p>
            </div>
          </div>
          <button
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        <PointInput label="Batida Final" ruleKey="pointsForEnding" rules={localRules} isHost={isHost} onChange={updatePoints} />
                        <PointInput label="Canastra Limpa" ruleKey="pointsCleanCanastra" rules={localRules} isHost={isHost} onChange={updatePoints} />
                        <PointInput label="Canastra Suja" ruleKey="pointsDirtyCanastra" rules={localRules} isHost={isHost} onChange={updatePoints} />
                        <PointInput label="Canastra de 500 (13 cartas)" ruleKey="pointsKingCanastra" rules={localRules} isHost={isHost} onChange={updatePoints} />
                        <PointInput label="Canastra Real (14 cartas)" ruleKey="pointsAceCanastra" rules={localRules} isHost={isHost} onChange={updatePoints} />
                        <PointInput label="Morto não pego" ruleKey="penaltyDeadPileNotTaken" rules={localRules} isHost={isHost} onChange={updatePoints} />
                    </div>
                </div>
              </div>
          ) : (
        ...
        <div className="p-6 md:p-8 bg-black/20 border-t border-white/5">
          <StyledButton onClick={onClose} className="w-full h-14 text-sm font-black tracking-[0.2em]">

...
const TeamList = ({
    ...
    <div className={`flex-1 min-w-0 bg-black/20 rounded-xl p-4 border border-white/5 flex flex-col gap-2 ${color}`}>
      <h3 className="text-xs font-black uppercase tracking-widest opacity-70 mb-2 border-b border-white/5 pb-2 truncate">
        {teamName}
      </h3>
      {players.length > 0 ? (
        players.map((p, idx) => (
          <div key={idx} className="flex items-center justify-between text-sm font-bold group/item gap-2">
             <div className="flex items-center gap-2 overflow-hidden min-w-0 flex-1">
                {p.isBot ? <Bot size={14} className="opacity-50 shrink-0" /> : <Users size={14} className="opacity-50 shrink-0" />}
                <span className="truncate flex-1" title={p.userName}>{p.userName}</span>
                {p.slotId === 1 && <span className="text-[9px] bg-yellow-500 text-black px-1 rounded font-black shrink-0">LÍDER</span>}
                {p.isBot && <span className="text-[9px] bg-white/10 px-1 rounded text-white/40 shrink-0">BOT</span>}
                {!p.isBot && p.isReady && <span className="text-[9px] bg-green-500 text-black px-1 rounded font-black shrink-0">PRONTO</span>}
                {!p.isBot && !p.isReady && p.slotId !== 1 && <span className="text-[9px] bg-yellow-500/10 text-yellow-500 px-1 rounded font-black border border-yellow-500/20 shrink-0">...</span>}
             </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Aguardando</span>
              </div>
            ...
            <div className="flex items-center gap-3 self-start md:self-center">
               <div className="flex flex-col items-end gap-1">
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Link de Convite</span>
                  <button 
                  ...
                  >
                    {copied ? (
                      <>
                        ...
                        <span className="text-xs font-black uppercase tracking-wider">Copiado!</span>
                      </>
                    ) : (
                      <>
                        <Share2 size={16} />
                        <span className="text-xs font-black uppercase tracking-wider">Copiar Link</span>
                      </>
                    )}
                  </button>
               </div>
        ...
        <div className="bg-black/40 backdrop-blur-xl p-1 rounded-[2.5rem] border border-white/5 shadow-2xl">
          <div className="bg-slate-900/40 p-6 md:p-8 rounded-[2.2rem]">
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-8 relative">
              <TeamList teamName="Equipe 1" players={team1} color="border-blue-500/20 bg-blue-500/5 text-blue-200" myPlayerNumber={my_player_number} />
              ...
              <TeamList teamName="Equipe 2" players={team2} color="border-red-500/20 bg-red-500/5 text-red-200" myPlayerNumber={my_player_number} />
            </div>
            ...
            <div className="flex flex-col gap-4">
               ...
               <div className="bg-black/40 h-2 w-full rounded-full overflow-hidden p-0.5 border border-white/5">
                  <div 
                    ...
                    style={{ width: `${(Object.keys(players_data).length / maxPlayers) * 100}%` }}
                  ></div>
               </div>
              ...
              {my_player_number === 1 && missingCount === 0 ? (
                  <div className="animate-fade-in mt-2">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                            ...
                            {rules.pointsCleanCanastra}pts Limpa
                            </span>
                            <span className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[9px] font-bold text-slate-500 uppercase tracking-tight">
                            {rules.pointsForEnding}pts Batida
                            </span>
                      </div>

                      <div className={!allOthersReady ? "opacity-30 pointer-events-none grayscale" : ""}>
                      ...
                      <div className="w-full flex flex-col gap-3 mt-2">
                          ...
                          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between mb-2">
                             <div className="flex flex-col gap-0.5">
                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Meta da Partida</span>
                                <span className="text-sm font-bold text-yellow-500 uppercase">
                                    {!winCondition ? "Partida Rápida" : 
                                     winCondition.type === "POINTS" ? `${winCondition.value} Pontos` : 
                          ...
                          <p className="text-center text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                               {myPlayerReady ? "Aguardando líder iniciar a partida..." : "Prepare-se para o jogo"}
                          </p>
                      </div>
                  ) : (
                       <div className="w-full bg-white/5 text-slate-500 py-6 rounded-2xl font-black text-xs uppercase tracking-[0.3em] border border-white/5 mt-2 text-center animate-pulse">
                          Aguardando Jogadores... ({Object.keys(players_data).length}/{maxPlayers})
                      </div>
                  )
              )}

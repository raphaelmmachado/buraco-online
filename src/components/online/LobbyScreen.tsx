    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-6 font-sans">
      <div className="text-center mb-12 animate-fade-in">
        ...
        <div className="flex items-center justify-center gap-2">
            ...
            <p className="text-lg text-slate-400 tracking-widest uppercase">Aguardando Jogadores</p>
        </div>
      </div>

      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-lg border border-slate-700">
        <div className="flex justify-between items-center mb-8 border-b border-slate-700 pb-4">
            <h2 className="text-xl font-bold text-slate-200">Jogadores na Mesa</h2>
        ...
        <div className="grid grid-cols-1 gap-4 mb-10">
          {connectedPlayers.map((player, idx) => (
            <div key={idx} className="flex items-center gap-4 bg-slate-900/50 p-4 rounded-xl border border-slate-700 animate-slide-up">
                ...
                <div>
                    <p className="font-bold text-slate-100">{player.userName}</p>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest">Jogador Conectado</p>
                </div>
                <div className="ml-auto">
                    <span className="text-green-500 text-xs font-bold uppercase">Pronto</span>
                </div>
            </div>
          ))}

          {[...Array(missingCount)].map((_, i) => (
            <div key={i} className="flex items-center gap-4 bg-slate-800/50 p-4 rounded-xl border border-slate-700 border-dashed opacity-40">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-slate-600 uppercase tracking-widest text-xs">Vaga disponível</p>
                </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center">
            <p className="text-slate-400 text-sm italic mb-6">
                {missingCount > 0 
                    ? `Faltam ${missingCount} jogador${missingCount > 1 ? 'es' : ''} para a partida começar.` 
